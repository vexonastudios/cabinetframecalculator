    ...
    <div className="bg-slate-900 border-2 border-emerald-500/80 rounded-2xl p-4 sm:p-5 shadow-2xl shadow-emerald-950/60 ring-1 ring-emerald-500/30 space-y-4">
            ...
            {cutList.map((item) => (
              <tr key={item.id} className="hover:bg-emerald-950/30 transition-colors">
                ...
                <td className="py-3.5 px-4">
                  <span className="inline-flex items-center justify-center px-2.5 py-1 rounded-lg bg-emerald-500 text-slate-950 font-mono font-black text-sm shadow">
                    {item.quantity}×
                  </span>
                </td>
                ...
                <td className="py-3.5 px-4 font-mono font-black text-emerald-400 text-base">
                  {item.lengthFraction}
                </td>
      ...
      <div className="md:hidden space-y-3">
        {cutList.map((item) => (
          <div 
            key={item.id} 
            className="bg-slate-950 rounded-xl border-2 border-emerald-500/60 border-l-8 border-l-emerald-500 p-4 shadow-lg space-y-3"
          >
            {/* Header row: Qty + Name + Copy */}
            <div className="flex justify-between items-center gap-2">
              <div className="flex items-center gap-2.5">
                <span className="inline-flex items-center justify-center px-3 py-1.5 rounded-lg bg-emerald-500 text-slate-950 font-mono font-black text-base shadow">
                  {item.quantity}×
                </span>
                <div>
                  <span className="font-extrabold text-white text-base block leading-tight">{item.name}</span>
            ...
            <div className="bg-emerald-950/40 rounded-xl p-3.5 border border-emerald-500/40 flex items-center justify-between">
              <div>
                <span className="text-[10px] uppercase font-black tracking-wider text-emerald-400 block mb-0.5">
                  CUT LENGTH TO MAKE:
                </span>
                <div className="flex items-baseline gap-2">
                  <span className="font-mono font-black text-emerald-400 text-2xl tracking-tight">
                    {item.lengthFraction}
                  </span>
